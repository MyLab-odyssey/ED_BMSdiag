...
void canDiag::getBMSdata(BatteryDiag_t *myBMS) {
}
...
//--------------------------------------------------------------------------------
void canDiag::begin(MCP_CAN *myCANobj, CTimeout *myCAN_TimeoutObj) {
  //Set Pointer to MCP_CANobj
  myCAN0 = myCANobj;
  myCAN_Timeout = myCAN_TimeoutObj;
...
boolean canDiag::getBatteryCapacity(BatteryDiag_t *myBMS, boolean debug_verbose) {
  ...
  if(items){
    ...
    myBMS->HVoff_time = (unsigned long) data[5] * 65535 + data[6] * 256 + data[7];
    myBMS->HV_lowcurrent = (unsigned long) data[9] * 65535 + data[10] * 256 + data[11];
